...
def add_data_for_name(name_data, year, rank, name):
    ...
    """
    if name not in name_data:  # 加入「未存過」的名字
        name_data[name] = {year: rank}
    else:
        if year in name_data[name]:  # 有重複年份
            existing_rank = int(name_data[name][year])
            new_rank = int(rank)
            if new_rank < existing_rank:  # 留下排名高者
                name_data[name][year] = rank  # 加入
        else:  # 沒有重複年份、直接添加到對應名字
            name_data[name][year] = rank
